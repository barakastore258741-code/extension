...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { license_key, lovable_token, project_id, browser_session_id } = body;
    ...
    if (!lovable_token || !project_id) {
      return new Response(JSON.stringify({ ok: false, error: "Missing token or project_id" }), {
        status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }

    const watermarkPrompt = "Remove all 'Built with Lovable' watermarks, badges, and branding from the project. Make sure the footer, header, and all visible UI elements do not show any Lovable attribution. Keep everything else exactly as is.";

    const resp = await fetch(`https://lovable-api.com/projects/${project_id}/messages`, {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({ content: watermarkPrompt, model: "gpt-4o" }),
    });
